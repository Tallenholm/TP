...
fn lexes_function_and_expression() {
    ...
    let source = SourceFile::new("main.tp", "fn add(a: i64, b: i64) -> i64 { a + b }");
    ...
    assert_eq!(
        kinds,
        vec![
            Fn, Identifier, LParen, Identifier, Colon, Identifier, Comma, Identifier, Colon,
            Identifier, RParen, Arrow, Identifier, LBrace, Identifier, Plus, Identifier, RBrace,
            Eof,
        ]
    );
...
fn invalid_character_reports_error_and_recovers() {
    ...
    assert_eq!(
        kinds,
        vec![
            Let, Identifier, Equal, Integer, Let, Identifier, Equal, Integer, Eof
        ]
    );
}
