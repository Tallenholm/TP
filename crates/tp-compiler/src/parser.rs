...
impl<'a> Parser<'a> {
    ...
    fn parse_enum(&mut self) -> Option<EnumDecl> {
        ...
        while !self.at(TokenKind::RBrace) && !self.at(TokenKind::Eof) {
            ...
            variants.push(VariantDecl {
                name: variant_name,
                payload,
                span: Span::new(self.source.id(), variant_start, end),
            });
            if !self.consume(TokenKind::Comma) && !self.at(TokenKind::RBrace) {
        ...
        loop {
            ...
            if self.consume(TokenKind::Dot) {
                ...
                expr = Expr {
                    ...
                    span: Span::new(self.source.id(), start, field_token.span.end),
                };
                continue;
            }
            if self.looks_like_struct_literal() {
                let ExprKind::Name(type_name) = &expr.kind else {
                    break;
                };
                ...
                while !self.at(TokenKind::RBrace) && !self.at(TokenKind::Eof) {
                    let name_token =
                        self.expect(TokenKind::Identifier, "expected struct field name")?;
    ...
    fn looks_like_struct_literal(&self) -> bool {
        self.at(TokenKind::LBrace)
            && self.lookahead_kind(1) == TokenKind::Identifier
            && self.lookahead_kind(2) == TokenKind::Colon
    }
    ...
    fn parse_primary(&mut self) -> Option<Expr> {
        ...
        match token.kind {
            TokenKind::Integer => {
                self.advance();
                Some(Expr {
                    kind: ExprKind::Integer(self.text(&token).parse::<i64>().ok()?),
                    span: token.span,
                })
            }
            TokenKind::Float => {
                self.advance();
                Some(Expr {
                    kind: ExprKind::Float(self.text(&token).parse::<f64>().ok()?),
                    span: token.span,
                })
            }
            TokenKind::String => {
    ...
    fn parse_match_expression(&mut self) -> Option<Expr> {
        ...
        while !self.at(TokenKind::RBrace) && !self.at(TokenKind::Eof) {
            ...
            arms.push(MatchArm {
                pattern,
                body,
                span: Span::new(self.source.id(), arm_start, arm_end),
            });
            if !self.consume(TokenKind::Comma) && !self.at(TokenKind::RBrace) {
    ...
    fn parse_pattern(&mut self) -> Option<Pattern> {
        ...
        match token.kind {
            TokenKind::Identifier => {
                ...
                if self.consume(TokenKind::LParen) {
                    ...
                    return Some(Pattern {
                        ...
                        span: Span::new(self.source.id(), token.span.start, close.span.end),
                    });
                }
                Some(Pattern {
            ...
            TokenKind::Integer => {
                self.advance();
                Some(Pattern {
                    kind: PatternKind::Integer(self.text(&token).parse().ok()?),
                    span: token.span,
                })
            }
            TokenKind::String => {
                self.advance();
                Some(Pattern {
                    kind: PatternKind::String(decode_string(self.text(&token))),
                    span: token.span,
                })
            }
            TokenKind::True | TokenKind::False => {
                self.advance();
                Some(Pattern {
                    kind: PatternKind::Bool(token.kind == TokenKind::True),
                    span: token.span,
                })
            }
            _ => {
                self.error_current("expected match pattern");
                None
            }
        }
    }
    ...
    fn lookahead_kind(&self, offset: usize) -> TokenKind {
        self.tokens
    ...
    fn error_current(&mut self, message: impl Into<String>) {
        self.diagnostics
            .push(Diagnostic::error("TP-E0100", message).with_primary(self.current_span()));
    }
}
